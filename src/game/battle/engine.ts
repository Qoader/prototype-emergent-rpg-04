...
export const createBattle = (goblinId: string, scene?: BattleState['scene']): BattleState => { const player = createCombatant('player', 'player', { col: 2, row: 4 }); const goblin = createCombatant(goblinId, 'goblin', { col: 6, row: 4 }); return { ...BATTLE_GRID, combatants: { player: player, [goblinId]: goblin }, turnOrder: ['player', goblinId], activeId: 'player', turn: 1, outcome: null, log: [{ kind: 'turn-start', actorId: 'player', turn: 1 }], scene }; };
...
export function advanceTurn(state: BattleState, events: BattleEvent[]): void {
  ...
  state.activeId = state.turnOrder[nextIndex]!; state.turn += nextIndex === 0 ? 1 : 0;
  const next = state.combatants[state.activeId]!; next.ap = next.maxAp; next.mp = next.maxMp;
...
export function applyBattleCommand(input: BattleState, command: BattleCommand): BattleTransition {
  ...
  if (command.kind === 'move') { const cells = reachable(state, actor.id); const node = cells.get(`${command.destination.col},${command.destination.row}`); if (!node || node.distance === 0) return { state: input, error: 'Destination is unreachable', events: [] }; const path = [node.point]; let cursor = node; while (cursor.previous) { cursor = cells.get(cursor.previous)!; path.unshift(cursor.point); } actor.position = { ...command.destination }; actor.mp -= node.distance; events.push({ kind: 'move', actorId: actor.id, from: { ...input.combatants[actor.id]!.position }, to: { ...actor.position }, cost: node.distance, path }); }
  ...
  if (command.kind !== 'end-turn' && !state.outcome && actor.ap === 0 && actor.mp === 0) advanceTurn(state, events);
