...
export const chooseGoblinCommand = (state: BattleState): BattleCommand => { const goblin = state.combatants[state.activeId]!; const player = Object.values(state.combatants).find((c) => c.side === 'player')!; if (goblin.ap > 0 && distance(goblin.position, player.position) === 1) return { kind: 'attack', actorId: goblin.id, targetId: player.id }; if (goblin.mp <= 0) return { kind: 'end-turn', actorId: goblin.id }; const cells = search(state, goblin.id); const goals = [{ col: player.position.col, row: player.position.row - 1 }, { col: player.position.col + 1, row: player.position.row }, { col: player.position.col, row: player.position.row + 1 }, { col: player.position.col - 1, row: player.position.row }]; let best: { point: { col: number; row: number }; distance: number } | undefined; for (const goal of goals) { const candidate = cells.get(`${goal.col},${goal.row}`); if (candidate && (!best || candidate.distance < best.distance)) best = { point: candidate.point, distance: candidate.distance }; } if (best && best.distance > 0) { const node = best.point; let current = cells.get(`${node.col},${node.row}`)!; const path = [node]; while (current.previous) { const prior = cells.get(current.previous)!; path.unshift(prior.point); current = prior; } return { kind: 'move', actorId: goblin.id, destination: path[Math.min(goblin.mp, path.length - 1)]! }; } return { kind: 'end-turn', actorId: goblin.id }; };
