...
export const createCombatant = (id: string, side: 'player' | 'goblin', position: Point, kind: BattleCombatant['kind'] = side === 'goblin' ? 'goblin' : id === 'player' ? 'player' : 'adventurer'): BattleCombatant => {
  const stats = side === 'player' ? PLAYER_BATTLE_STATS : GOBLIN_BATTLE_STATS;
  return { id, side, kind, control: kind === 'player' ? 'human' : 'ai', eligibleFromRound: 1, position: { ...position }, ...stats, hp: stats.maxHp, ap: stats.maxAp, mp: stats.maxMp };
};
