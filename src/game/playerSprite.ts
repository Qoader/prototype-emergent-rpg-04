...
function frame(animation: PlayerAnimation, facing: Facing, index: number, adventurer = false): Graphics {
  const g = new Graphics();
  const bob = animation === 'idle' ? (index ? -1 : 0) : index % 2;
  const step = animation === 'walk' ? ([0, 1, 0, -1][index] ?? 0) : 0;
  const skin = '#f2b38f'; const hair = '#3c2630'; const coat = adventurer ? '#55734f' : '#4c6f8a'; const trim = adventurer ? '#b38b55' : '#d9b36c'; const boot = '#3a2c32';
...
function createCharacterSprite(adventurer: boolean): PlayerSprite {
  ...
  for (const facing of directions) for (let index = 0; index < 4; index++) { const g = frame('walk', facing, index, adventurer); g.visible = false; frames.set(key('walk', facing, index), g); view.addChild(g); }
  for (const facing of directions) for (let index = 0; index < 2; index++) { const g = frame('idle', facing, index, adventurer); g.visible = false; frames.set(key('idle', facing, index), g); view.addChild(g); }
