...
function frame(animation: PlayerAnimation, facing: Facing, index: number, adventurer = false, goblin = false): Graphics {
  ...
  if (adventurer && facing !== 'south') g.rect(-18, -23 + bob, 5, 12).fill('#70452f');
...
function createCharacterSprite(adventurer: boolean, goblin = false): PlayerSprite {
  ...
  for (const facing of directions) for (let index = 0; index < 4; index++) { const g = frame('walk', facing, index, adventurer, goblin); g.visible = false; frames.set(key('walk', facing, index), g); view.addChild(g); }
  for (const facing of directions) for (let index = 0; index < 2; index++) { const g = frame('idle', facing, index, adventurer, goblin); g.visible = false; frames.set(key('idle', facing, index), g); view.addChild(g); }
...
export function createPlayerSprite(): PlayerSprite { return createCharacterSprite(false); }
export function createAdventurerSprite(): PlayerSprite { return createCharacterSprite(true); }
export function createGoblinSprite(): PlayerSprite { return createCharacterSprite(false, true); }
