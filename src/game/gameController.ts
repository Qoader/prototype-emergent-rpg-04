...
export function createGameController(map: WorldMap, tiles?: TileReader) {
  ...
  const finish = () => { if (session.kind !== 'battle' || !session.battle.outcome) return; const { encounter, battle: finished } = session; if (finished.outcome === 'victory') goblins.remove(encounter.goblinId); if (finished.outcome === 'defeat') relocate(checkpoints.resolve()); session = { kind:'result', encounter, battle:finished }; aiWait = 0; notify(); };
