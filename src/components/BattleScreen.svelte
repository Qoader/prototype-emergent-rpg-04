...
{#if battle}
  <section class="battle" aria-label="Tactical battle">
    <div class:failed={battleArtFailed} class="battle-art" bind:this={battleHost} aria-hidden="true"></div>
...
<style>
  .battle { position: absolute; inset: 0; z-index: 5; display: grid; place-content: center; gap: 1rem; padding: 1rem; background: rgba(10, 16, 27, .94); color: #f5f0dc; font-family: system-ui, sans-serif; overflow: auto; } .battle-art { position: absolute; inset: 0; pointer-events: none; } .battle-art.failed { display: none; }
